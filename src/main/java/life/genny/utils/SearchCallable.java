...
public class SearchCallable implements Callable<QBulkMessage> {
  ...
  public SearchCallable(
      TableUtils tableUtils, SearchEntity searchBE, BaseEntityUtils beUtils, Boolean cache) {
  ...
  public QBulkMessage call() {
    ...
    final String templateSearchCode =
        searchBE.getCode().replaceFirst("_" + beUtils.getGennyToken().getJTI().toUpperCase(), "");
      ...
      if (pageStart == 0) {
        ...
        if (qbm1 == null) {
          ...
          if (qbm1.getMessages()[0].getParentCode() != null) {
            qbm1.getMessages()[0].setParentCode(
                templateSearchCode
                    + "_"
                    + beUtils.getGennyToken().getJTI().toUpperCase()); // fix to current session
          } else {
            Arrays.stream(qbm1.getMessages()[0].getItems())
                .forEach(
                    i ->
                        i.setCode(
                            templateSearchCode
                                + "_"
                                + beUtils.getGennyToken().getJTI().toUpperCase()));
          }
          if (qbm1.getMessages().length > 0) {
            if (qbm1.getMessages()[1].getParentCode() != null) {
              qbm1.getMessages()[1].setParentCode(
                  templateSearchCode
                      + "_"
                      + beUtils.getGennyToken().getJTI().toUpperCase()); // fix to current session
            } else {
              Arrays.stream(qbm1.getMessages()[1].getItems())
                  .forEach(
                      i ->
                          i.setCode(
                              templateSearchCode
    ...
    log.info(
        "Finished "
            + (usingCache ? "Using cache " : "with no caching ")
            + " - Search with "
            + qbm1.getMessages().length
            + " items ");
    return ret;
  }
}
