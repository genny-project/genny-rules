...
public class JbpmInitListener /* extends WorkingMemoryLogger */ implements ProcessEventListener {
  ...
  public void beforeProcessStarted(ProcessStartedEvent event) {
    ...
    if (this.userToken != null) {
      ...
      if ((existing == null) || ((!existing.getJTI().equals(userToken.getJTI())))) { // save
        // adding a
        // duplicate
            ...
            obj -> {
              if (obj instanceof Long) {
                ...
                if (("PER_SERVICE".equals(gennyToken.getCode()))
                    || ("serviceToken".equals(gennyToken.getCode()))) {
                  ...
                  GennyToken existing = (GennyToken) process.getVariable("userToken");
                  if ((existing == null)
                      || ((!existing.getJTI().equals(gennyToken.getJTI())))) { // save
                    // adding
                    // a
                    // duplicate
                    ...
                    if (processId.contains("ession")) { // only bother with session type workflows
                      log.info(
                          "JbpmListener: userToken "
                              + gennyToken.getUserCode()
                              + " processId="
                ...
                //				printProcessText(process, gennyToken, "FOUND GennyToken  " +
                // gennyToken.getCode());
                ...
                //				printProcessText(process, gennyToken, "FOUND OBJ " +
  ...
  public void beforeProcessCompleted(ProcessCompletedEvent event) {
    ...
    //              + ", Process definition ID: " + process.getProcessId() + ", Process name: "
    //              + process.getProcessName() + ", Process state: " + process.getState() + ",
    // Parent process ID: "
    //              + process.getParentProcessInstanceId());
    //
  ...
  private void processStart(WorkflowProcessInstance process, GennyToken gennyToken) {
    ...
    try {
      ...
      // sub process)
      String indent =
          process.getParentProcessInstanceId() < 0
              ? ""
              : (process.getParentProcessInstanceId() + ">>>>>>>>>>");

      String starttext =
          RulesUtils.executeRuleLogger(
                  indent + ">>>>>>>>>> START PROCESS ",
                  processDetails(process, gennyToken),
                  RulesUtils.ANSI_RED,
  ...
  private void processEnd(
      WorkflowProcessInstance process, GennyToken gennyToken, double differenceMs) {
    ...
    try {
      ...
      String text =
          processDetails(process, gennyToken) + "  time=" + differenceMs + " ms"; // This is
      // faster
      // than
      // calling
      // getUser()
      ...
      RulesUtils.println(
          "Error in process: " + gennyToken.getRealm() + ":" + process.getProcessName(),
          "ANSI_RED");
    }
  }

  private void printProcessText(
      WorkflowProcessInstance process, GennyToken gennyToken, final String text) {
    ...
    try {
      ...
      String starttext =
          RulesUtils.executeRuleLogger(
                  indent + ">>>>>>>>>>     PROCESS:" + processDetails(process, gennyToken),
                  text,
                  RulesUtils.ANSI_RED,
                  RulesUtils.ANSI_YELLOW)
              + (GennySettings.devMode ? "" : RulesUtils.ANSI_RED)
   ...
   *
   * <p>This method also makes sure that the entity manager used for persisting the entity, joins
   * the existing JTA transaction.
   *
   ...
   * associated with the entity manager/persistence context are detached.
   *
   * <p>Obviously, if the transaction returned by the {@link #joinTransaction(EntityManager)} method
   * is null, nothing is done with the transaction parameter.
   *
  ...
  public void logEventCreated(LogEvent logEvent) {
    switch (logEvent.getType()) {
      ...
      case LogEvent.AFTER_VARIABLE_INSTANCE_CHANGED:
        ...
        // addVariableLog(variableEvent.getProcessInstanceId(),
        // variableEvent.getProcessId(), variableEvent.getVariableInstanceId(),
